...
p.add_argument('--data', type=str, default='./', help='dataset')
...
opt['r'] = comm.Get_rank()
opt['n'] = comm.Get_size()
##opt['n'] = 10 ## added JL 12/05
print('This is the number of replicas form arguments:', opt['n'])
...
print('Parameters used:', opt)
print(' with n=num replicas=', opt['n'])
...
def train(e):
    ...
    for b in range(opt['nb']):
        for l in range(opt['L']):
            ...
            if opt['cuda']:
                ##x, y = x.cuda(async=True), y.cuda(async=True) fix just call cuda
                x, y = x.cuda(), y.cuda()
